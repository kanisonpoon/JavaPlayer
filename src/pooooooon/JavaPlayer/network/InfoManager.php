...
class InfoManager
{
	...
	public function __construct(InfoThread $thread, int $port, string $interface, string $description = "", string $favicon = null)
	{
		...
		if (!$this->socket) {
			$this->logger->critical("[JavaPlayer] **** FAILED TO BIND TO " . $interface . ":" . $port . "!");
			$this->logger->critical("[JavaPlayer] Perhaps a server is already running on that port?");
			exit(1);
		}
